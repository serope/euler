...
def has_commands():
    """Return True if the necessary commands/compilers are present."""
    commands = [
        ['make', '--version'],
        ['gcc', '--version'],
        ['go', 'version'],
    ...
    for c in commands:
        failmessage = '"{} {}" failed'.format(c[0], c[1])
        try:
            process = subprocess.run(c, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except:
            print(failmessage)
            return False
...
def build_solution(solution_filename: str) -> int:
    ...
    """
    pair = solution_filename.split('.')
    cmd = commands[pair[1]].replace('X', pair[0])
    cmd = cmd.split(' ')
...
def get_build_command(solution: str) -> str:
    ...
    pair = solution.split('.')
    return commands[pair[1]].replace('X', pair[0])
